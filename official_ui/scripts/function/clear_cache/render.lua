local C = load_function("clear_cache/constants.lua")
local L = load_function("clear_cache/layout.lua")
...
local function draw_title(root_state)
  local title = L.language(root_state, "CLEAR_CACHE_TITLE", C.DEFAULT_TEXT.title)
...
local function build_lines(root_state, started_at)
  local lines = {}
  local warn = L.language(root_state, "CLEAR_CACHE_WARN", C.DEFAULT_TEXT.warn)
  for _, line in ipairs(L.wrap(warn, 72)) do
    lines[#lines + 1] = { text = line, color = C.WARN_COLOR }
  end

  local dir = root_state.dir or {}
  local cache_path = L.language(root_state, "CLEAR_CACHE_CACHE_PATH", C.DEFAULT_TEXT.cache_path)
    .. tostring(dir.cache_dir or "")
  local log_path = L.language(root_state, "CLEAR_CACHE_LOG_PATH", C.DEFAULT_TEXT.log_path)
    .. tostring(dir.log_dir or "")
  lines[#lines + 1] = { text = "", color = C.TITLE_COLOR }
  lines[#lines + 1] = { text = cache_path, color = C.TITLE_COLOR }
  lines[#lines + 1] = { text = log_path, color = C.TITLE_COLOR }

  local remain = remaining_seconds(started_at)
  local confirm = L.language(root_state, "CLEAR_CACHE_CONFIRM", C.DEFAULT_TEXT.confirm)
  local cancel = L.language(root_state, "CLEAR_CACHE_CANCEL", C.DEFAULT_TEXT.cancel)
  lines[#lines + 1] = { text = "", color = C.TITLE_COLOR }
  lines[#lines + 1] = { text = C.DEFAULT_TEXT.cancel_key .. " " .. cancel, color = C.CANCEL_COLOR }
  if remain > 0 then
    lines[#lines + 1] = {
      text = C.DEFAULT_TEXT.confirm_key .. " " .. confirm .. " " .. tostring(remain) .. L.language(root_state, "CLEAR_CACHE_SECOND", C.DEFAULT_TEXT.second),
