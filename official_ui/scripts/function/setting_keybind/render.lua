local C = load_function("setting_keybind/constants.lua")
local L = load_function("setting_keybind/layout.lua")
local Menu = load_function("setting_keybind/menu.lua")

local M = {}

local function draw_title(root_state)
  local title = L.language(root_state, "SETTING_KEYBIND_LIST_TITLE", C.DEFAULT_TEXT.title)
...
local function draw_action_line(root_state)
  local select_text = L.language(root_state, "SETTING_KEYBIND_LIST_SELECT", C.DEFAULT_TEXT.select)
  local confirm_text = L.language(root_state, "SETTING_KEYBIND_LIST_CONFIRM", C.DEFAULT_TEXT.confirm)
  local back_text = L.language(root_state, "SETTING_KEYBIND_LIST_BACK", C.DEFAULT_TEXT.back)
