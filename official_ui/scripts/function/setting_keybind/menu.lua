  ...
  return {
    {
      ...
      label = L.language(root_state, "SETTING_KEYBIND_LIST_GLOBAL", C.DEFAULT_TEXT.global),
    },
    {
      key = C.DEFAULT_TEXT.option2,
      label = L.language(root_state, "SETTING_KEYBIND_LIST_SYSTEM", C.DEFAULT_TEXT.system),
    },
    {
      key = C.DEFAULT_TEXT.option3,
      label = L.language(root_state, "SETTING_KEYBIND_LIST_GAME", C.DEFAULT_TEXT.game),
    },
  }
end
...
local function selected_index(root_state)
  if type(root_state) == "table" and type(root_state.select) == "number" then
    return math.max(1, math.min(3, math.floor(root_state.select)))
  end
  return 1
end
