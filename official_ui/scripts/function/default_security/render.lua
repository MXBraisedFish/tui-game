local C = load_function("default_security/constants.lua")
local L = load_function("default_security/layout.lua")
...
local function draw_title(root_state)
  local title = L.language(root_state, "DEFAULT_SECURITY_TITLE", C.DEFAULT_TEXT.title)
...
local function build_lines(root_state, started_at)
  local lines = {}
  local warn = L.language(root_state, "DEFAULT_SECURITY_WARN", C.DEFAULT_TEXT.warn)
  ...
  local cancel = L.language(root_state, "DEFAULT_SECURITY_CANCEL", C.DEFAULT_TEXT.cancel)
  local close_permanent = L.language(root_state, "DEFAULT_SECURITY_CLOSE_PERMANENT", C.DEFAULT_TEXT.close_permanent)
  local second = L.language(root_state, "DEFAULT_SECURITY_SECOND", C.DEFAULT_TEXT.second)

  lines[#lines + 1] = { text = "", color = C.TITLE_COLOR }
  lines[#lines + 1] = { text = C.DEFAULT_TEXT.cancel_key .. " " .. cancel, color = C.CANCEL_COLOR }
  if remain > 0 then
    lines[#lines + 1] = {
      text = C.DEFAULT_TEXT.close_permanent_key .. " " .. close_permanent .. " " .. tostring(remain) .. second,
      color = C.DISABLED_COLOR
    }
  else
    lines[#lines + 1] = {
      text = C.DEFAULT_TEXT.close_permanent_key .. " " .. close_permanent,
      color = C.CONFIRM_COLOR
    }
  end
  return lines
end
