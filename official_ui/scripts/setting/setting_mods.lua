local Render = load_function("mod_list/render.lua")
...
local function normalize_list_mode(mode)
  if mode == "brief" then
    return "brief"
  end
  return "full"
...
local function toggle_list_mode(mode)
  if normalize_list_mode(mode) == "full" then
    return "brief"
  end
  return "full"
end
...
local function normalize_lua_state(lua_state)
  ...
  lua_state.list_mode = normalize_list_mode(lua_state.list_mode)
  return lua_state
end
...
function handle_event(lua_state, event)
  ...
  if event.type == "action" then
    ...
    elseif event.name == "list" then
      lua_state.list_mode = toggle_list_mode(lua_state.list_mode)
    elseif event.name == "debug" then
      lua_state.toggle_debug = true
    elseif event.name == "safe_mode" then
      lua_state.toggle_safe_mode = true
