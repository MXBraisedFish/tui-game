local Render = load_function("mod_hub/render.lua")

local function normalize_select(select)
  if type(select) ~= "number" then return 1 end
  select = math.floor(select)
  if select < 1 then return 3 end
  if select > 3 then return 1 end
  return select
end
...
function handle_event(lua_state, event)
  ...
  if event.type == "action" then
    ...
    elseif event.name == "option1" then lua_state.select = 1
    elseif event.name == "option2" then lua_state.select = 2
    elseif event.name == "option3" then lua_state.select = 3
    end
  end
  return lua_state
end
