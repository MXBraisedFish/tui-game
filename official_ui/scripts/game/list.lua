local Render = load_function("mod_game_list/render.lua")
local State = load_function("mod_game_list/state.lua")
...
local function normalize_sort(sort)
  if sort == "author" or sort == "safe_mode" or sort == "toggle" then
    return sort
  end
  return "name"
end
...
local function normalize_lua_state(lua_state)
  ...
  if type(lua_state.mod_list) == "table" then
  ...
  lua_state.list_mode = lua_state.list_mode == "brief" and "brief" or "full"
  lua_state.toggle_debug = false
  lua_state.toggle_safe_mode = false
  return lua_state
end
...
function handle_event(lua_state, event)
  ...
  if event.type ~= "action" and event.type ~= "key" then return lua_state end
  if event.status ~= "press" then return lua_state end
  if lua_state.jump then
    if event.type == "action" then
      if event.name == "confirm" then
        if lua_state.user_page >= 1 and lua_state.user_page <= lua_state.pages then lua_state.page = lua_state.user_page end
        lua_state.user_page = 0; lua_state.jump = false
      elseif event.name == "return" then
        lua_state.user_page = 0; lua_state.jump = false
      end
    elseif event.type == "key" then
      local digit = tonumber(event.name)
      if digit ~= nil and digit >= 0 and digit <= 9 then append_user_page(lua_state, digit) end
    end
    return lua_state
  end
  if event.type == "action" then
    if event.name == "prev_option" then update_select(lua_state, -1)
    elseif event.name == "next_option" then update_select(lua_state, 1)
    elseif event.name == "prev_page" then update_page(lua_state, -1)
    elseif event.name == "next_page" then update_page(lua_state, 1)
    elseif event.name == "scroll_up" then lua_state.info_scroll = math.max(0, lua_state.info_scroll - 1)
    elseif event.name == "scroll_down" then lua_state.info_scroll = math.min(Render.max_info_scroll(lua_state), lua_state.info_scroll + 1)
    elseif event.name == "jump" then if lua_state.pages > 1 then lua_state.user_page = 0; lua_state.jump = true end
    elseif event.name == "order" then lua_state.order = toggle_order(lua_state.order)
    elseif event.name == "sort" then lua_state.sort = next_sort(lua_state.sort)
    elseif event.name == "confirm" then lua_state.confirm = true
    elseif event.name == "debug" then lua_state.toggle_debug = true
    elseif event.name == "safe_mode" then lua_state.toggle_safe_mode = true
    elseif event.name == "list" then lua_state.list_mode = lua_state.list_mode == "brief" and "full" or "brief"
