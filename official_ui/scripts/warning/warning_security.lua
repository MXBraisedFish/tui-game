local Render = load_function("default_security/render.lua")
...
function handle_event(lua_state, event)
  lua_state = lua_state or { close_permanent = false, back = false }
  ...
  if event.type == "action" and event.name == "close_permanent" then
    if event.status == "press" and ready_to_close_permanent() then
