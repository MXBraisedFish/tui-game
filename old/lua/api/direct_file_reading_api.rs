// 文件读取 API，安全读取游戏包内 assets/ 目录下的文件。提供 translate（包级 i18n 文本解析）、read_bytes, read_text, read_json, read_json_string, read_yaml, read_yaml_string, read_toml, read_toml_string, read_csv, read_csv_string, read_xml, read_xml_string 等函数。所有路径必须以 / 或 \ 开头（绝对逻辑路径），并经过安全检查防止目录穿越

use std::collections::BTreeMap; // 存储 XML 属性
use std::fs; // 文件读取
use std::io::ErrorKind; // 区分文件不存在等错误
use std::path::{Component, PathBuf}; // 路径规范化

use csv::StringRecord; // CSV 记录
use mlua::{Lua, Table, Value, Variadic}; // Lua 类型
use quick_xml::Reader; // XML 解析
...
// 注入 translate, read_bytes, read_text, read_json, read_json_string, read_yaml, read_yaml_string, read_toml, read_toml_string, read_csv, read_csv_string, read_xml, read_xml_string
...
// 根据当前游戏的包，尝试从 assets/lang/{lang}.json 或 en_us.json 中查找键值；失败返回 [missing-i18n-key: key] 并写警告
...
// 获取当前游戏的包描述符（GameDescriptor::package_info）
...
// 将逻辑路径（如 /data/foo.json）解析为 package.root_dir/assets/ 下的绝对路径，禁止 .. 和绝对路径，确保文件存在
...
// 加载语言字典并查找键，返回 Option<String> 或错误
...
// 将 CSV 文本转换为 JSON 数组（每行为字符串数组），使用 csv crate
...
// 将 XML 文本转换为 JSON 对象，结构为 {tag, attributes, children, text}，使用 quick_xml
...
// XML 属性转为 BTreeMap<String, String>
...
// 内部结构 XmlNode 用于构建 XML DOM 树，包含标签、属性、子节点和文本内容
#[derive(Default)]
struct XmlNode {
    tag: String,
