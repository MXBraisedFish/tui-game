// 随机数 API，提供多种随机数生成方式。包括临时随机数（无种子）、可种子随机数生成器（支持整数/浮点类型），并允许对生成器进行步数重置、删除、查询信息等管理。随机数生成器存储在 RuntimeBridges.randoms 中，受限最多 64 个

use std::collections::BTreeMap; // 存储生成器映射
use std::collections::hash_map::DefaultHasher; // 将种子字符串哈希为 u64
use std::hash::{Hash, Hasher}; // 哈希 trait

use mlua::{Lua, Table, Value, Variadic}; // Lua 类型
use rand::rngs::StdRng; // 随机数生成器（确定性）
use rand::{Rng, SeedableRng}; // 随机数生成 trait
...
const MAX_RANDOMS: usize = 64; // 最多同时存在的种子随机数生成器数量
const DEFAULT_RANDOM_MAX: i64 = 2_147_483_647; // 调用 random() 无参数时的默认最大值（i32 最大值）

// 线程安全的随机数存储（由 Mutex 保护）
#[derive(Default)]
...
// 注册 random, random_float, random_create, random_float_create, random_reset_step, random_kill, set_random_note, get_random_list, get_random_info, get_random_step, get_random_seed, get_random_type 等
