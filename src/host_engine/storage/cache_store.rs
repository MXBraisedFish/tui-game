...
impl CacheStore {
    ...
    pub fn clear_all(&self) -> CacheStoreResult<()> {
        ...
        ensure_cache_dirs()?;
        write_json_pretty(&cache_path(GAME_SCAN_CACHE_FILE), &GameModuleRegistry::default())?;
        write_json_pretty(&cache_path(SAVER_SCAN_CACHE_FILE), &json!({}))?;
        write_json_pretty(&cache_path(BOSS_SCAN_CACHE_FILE), &json!({}))?;
        write_json_pretty(&cache_path(LANGUAGE_UI_CACHE_FILE), &BTreeMap::<String, LanguageUiText>::new())?;
        Ok(())
    }
    ...
    T: for<'de> Deserialize<'de> + Default,
{
    fs::read_to_string(path)
        .ok()
        .and_then(|raw_json| serde_json::from_str::<T>(raw_json.trim_start_matches('\u{feff}')).ok())
        .unwrap_or_default()
}
