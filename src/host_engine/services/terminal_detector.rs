//! 终端能力自动检测器。
//!
//! 参考 ratatui-image 的 Picker 设计，三层检测：
//! 1. stdin/stdout 应答查询（Kitty / Sixel 支持）
//! 2. 环境变量推测（iTerm2、WezTerm 等不支持 stdin 应答的终端）
//! 3. 环境变量推测真彩支持（COLORTERM）
//!
//! 检测结果用于预填检测向导的选项，用户确认/修改后保存。
...
impl TerminalDetector {
  /// 执行完整检测（stdin 查询 + 环境变量）。
  /// 独立调用时会向当前 stdout 写入查询序列。
  pub fn detect() -> DetectionResult {
    let mut stdout = io::stdout();
    Self::detect_with_writer(&mut stdout)
  }

  /// 在已经进入 alt screen/raw mode 的终端内检测，避免查询序列泄露到原始屏幕。
  /// 必须在 crossterm event 监听线程启动前调用。
  ...
  fn detect_with_writer(stdout: &mut impl Write) -> DetectionResult {
    ...
    // ── 真彩检测（环境变量） ──
    result.truecolor = Self::detect_truecolor();

    // stdin 响应是权威结果；环境变量只用于无响应时兜底到 iTerm2 等协议。
  ...
  fn query_stdio_with_timeout(stdout: &mut impl Write, timeout: Duration) -> Option<ImageProtocol> {
    use std::sync::mpsc;
    use std::thread;
...
mod tests {
  ...
  #[test]
  fn test_detect_truecolor_from_env() {
    // 仅验证逻辑不 panic
    let _ = TerminalDetector::detect_truecolor();
  }
