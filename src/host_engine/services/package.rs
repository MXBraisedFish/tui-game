...
impl PackageService {
  ...
  fn read_package(
    ...
    let content = fs::read_to_string(&manifest_path)
      .map_err(|error| format!("faild to read package.json: {}", error))?;

    // 序列化内容
    // TODO: 将语言国际化
    let raw: RawPackageManifest = serde_json::from_str(&content)
    ...
    let title = raw
      .display
      .as_ref()
      .and_then(|display| display.title.clone())
      .unwrap_or_else(|| raw.package.clone());
