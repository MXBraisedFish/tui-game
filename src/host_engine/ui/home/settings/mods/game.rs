...
impl GamePackageUi {
  ...
  pub fn handle_event(&mut self, event: &UiEvent) -> Option<GamePackageCommand> {
    match event {
      ...
      UiEvent::Action(event) if event.state == KeyState::Pressed => match event.action.as_str() {
        ...
        "game_pack.confirm"
        | "game_pack.scroll_up"
        | "game_pack.scroll_down"
        | "game_pack.debug"
        | "game_pack.list"
        | "game_pack.safe_mode" => None,
  ...
  pub fn render(
    ...
    self.sync_entries(package.mod_games());
  ...
  pub fn compute_positions(&self, layout: &LayoutService, i18n: &I18nService) -> GamePackageLayout {
  ...
  fn sort_value(&self, entry: &PackageListEntry) -> String {
    match self.sort_field {
      GameSortField::Title => entry.title.clone(),
      GameSortField::Author => entry.author.clone(),
