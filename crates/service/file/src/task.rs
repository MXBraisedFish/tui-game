...
use tg_core_atomic_fs::{atomic_write, temporary_path};
use tg_core_sandbox_path::{
  SafeRelativePath, SandboxPathError, SandboxPathKind, resolve_sandbox_path,
};
...
impl<E: From<FileEvent> + Send + 'static> AsyncJob<E> for FileTask {
  fn run(
    self: Box<Self>,
    id: TaskId,
    events: &Sender<E>,
    ...
    run_file_task(id, *self, events)
  }

  fn write_target(&self, _id: TaskId) -> Option<(PathBuf, PathBuf)> {
    match self {
      FileTask::WriteText { path, .. }
      | FileTask::WriteBytes { path, .. }
      | FileTask::LuaWriteText { path, .. }
      | FileTask::LuaWriteBytes { path, .. }
      | FileTask::LuaCreateDir { path, .. }
      | FileTask::LuaRemove { path, .. } => Some((path.clone(), temporary_path(path))),
      FileTask::ReadText { .. }
      | FileTask::ReadBytes { .. }
      | FileTask::LuaReadText { .. }
      | FileTask::LuaReadBytes { .. }
      | FileTask::LuaListDir { .. }
      | FileTask::LuaLoadI18n { .. } => None,
    }
  }
}

fn run_file_task<E: From<FileEvent>>(
  task_id: TaskId,
  task: FileTask,
  event_tx: &Sender<E>,
) -> Result<(), String> {
  match task {
    ...
    FileTask::WriteText { path, text } => match atomic_write(&path, text.as_bytes(), true) {
      Ok(()) => {
        let _ = event_tx.send(FileEvent::WriteTextFinished { task_id, path }.into());
        Ok(())
      }
      Err(error) => {
        send_file_error(event_tx, task_id, path, error.to_string());
    ...
    FileTask::WriteBytes { path, bytes } => match atomic_write(&path, &bytes, true) {
      Ok(()) => {
        let _ = event_tx.send(FileEvent::WriteBytesFinished { task_id, path }.into());
        Ok(())
      }
      Err(error) => {
        send_file_error(event_tx, task_id, path, error.to_string());
    ...
    FileTask::LuaWriteText {
      ...
      Ok(()) => {
        let _ = event_tx.send(FileEvent::LuaWriteTextFinished { task_id, path }.into());
        Ok(())
      }
      Err(error) => {
        send_file_error(event_tx, task_id, path, error.clone());
    ...
    .and_then(|path| {
      ...
      Ok(path) => {
        let _ = event_tx.send(FileEvent::LuaCreateDirFinished { task_id, path }.into());
        Ok(())
      }
      Err(error) => {
        send_file_error(event_tx, task_id, path, error.clone());
    ...
    {
      Ok(path) => {
        let _ = event_tx.send(FileEvent::LuaRemoveFinished { task_id, path }.into());
        Ok(())
      }
      Err(error) => {
        send_file_error(event_tx, task_id, path, error.clone());
...
fn send_file_error<E: From<FileEvent>>(
  event_tx: &Sender<E>,
  task_id: TaskId,
  path: PathBuf,
  error: String,
...
mod tests {
  ...
  fn lua_directory_creation_and_removal_are_revalidated_and_reported() {
    ...
    assert!(matches!(
      event_rx.recv().unwrap(),
      FileEvent::LuaCreateDirFinished { task_id: TaskId(1), path }
        if path == created
    ));
    ...
    assert!(matches!(
      event_rx.recv().unwrap(),
      FileEvent::Failed { task_id: TaskId(2), path, error }
    ...
    assert!(matches!(
      event_rx.recv().unwrap(),
      FileEvent::LuaRemoveFinished { task_id: TaskId(3), path }
        if path == created
    ));
