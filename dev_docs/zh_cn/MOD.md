...
所有模组包文件必须放置在宿主执行目录下的 `tui-game-data/mod/` 目录中，按命名空间组织。
...
| `introduction` | <font color="#92cddc">string</font> \| <font color="#92cddc">key</font>                                         | 模组包简介，在模组包列表中展示，由开发者编写。可填写字符串或语言键。            |
...
> 每个动作可绑定单个按键或多个按键。宿主会将按键事件转换为动作事件，通过 `handle_event` 传递给脚本（事件类型 `action`）。
...
1. 将模组包的 `命名空间（namespace）`、`包名（package）`、`作者（author）` 按顺序拼接成一个字符串。
2. 对该字符串进行哈希运算，然后使用 Base62 编码。
3. 取编码结果的前 16 位字符作为最终编码。

> 上述过程可用以下伪代码表示：
> ```
> encoding = base62(hash(namespace + package + author)).substring(0, 16)
> uid = "mod_game_" + encoding
> ```

**稳定性**：只要 `命名空间`、`包名`、`作者` 三者保持不变，生成的 UID 就不会改变。这确保了模组包在不同环境中的一致性识别。

**符号**：只允许`0-9` `a-z` `A-Z`组成。
...
图标用于在模组包列表中展示，显示区域为 **4 行 × 8 列**（终端字符）。
...
头图用于在模组包详情的详细信息展示，显示区域为 **13 行 × 86 列**（终端字符）。
